...
**Ejercicio 11 Guia 3: Read y Write**
---
Se quiere simular la comunicación mediante pipes entre dos procesos mediante las syscalls read()
y write(), pero usando memoria compartida (sin usar file descriptors). Se puede pensar al pipe
como un buffer de tamaño N, donde en cada posición se le puede escribir un cierto mensaje. El read()
debe ser bloqueante en caso que no haya ningún mensaje, y si el buffer está lleno, el write() también
debe ser bloqueante. No puede haber condiciones de carrera y se puede suponer que el buffer tiene los
siguientes métodos: pop() (saca el mensaje y lo desencola), push() (agrega un mensaje al buffer).

    Asumo que el buffer es una cola y no una pila.

    buffer pseudoPipe = init_buffer(N);

    semaforo bufferConMensajes = sem_init(0); // Maximo N signaleadas
    semaforo bufferConEspacio = sem_init(N);

    semaforo mutex = sem_init(1);

    void read()
    {
        bufferConMensajes.wait();
        
        mutex.wait();
        
        pseudoPipe.pop();
        
        bufferConEspacio.signal();
    ...
    void write()
    {
        bufferConEspacio.wait();

        mutex.wait();
        
        pseudoPipe.push();
        
        bufferConMensajes.signal();

        mutex.signal();
    }


**Ejercicio 12 Guia 3: El Lavarropa**
---

    #define forn(i,n) for (int i=0; i< n; i++)

    semaforo lavarropaLlena = sem_init(0);
    semaforo esperarLavada = sem_init(0);
    semaforo lavarropaLista = sem_init(0);
    semaforo lavarropaVacia = sem_init(0);

    semaforo mutex = sem_init(1);

    int cantidadDePrendasCargadas = 0;
    ...
    void lavarropa()
    {
        while(1)
        {
            ...
            forn(i, 10) lavarropaLista.signal();
            ...
            forn(i,10) {esperarLavada.signal();}
    ...
    {   
        ...
        entroAlLavarropa();
        cantidadDePrendasCargadas++;
        if(cantidadDePrendasCargadas == 10) lavarropaLlena.signal();        
        ...
        mutex.wait();
        cantidadDePrendasCargadas--;
        if (cantidadDePrendasCargadas == 0) lavarropaVacia.signal();
        mutex.signal();        
